...
void GPIOB_CONFIG(void)
{
	...
	GPIOB_MODER &= ~(0x01<<28); //clear PB13th pin ,positon is 27,26  (01)
	GPIOB_MODER |=(0x01<<28);		//set PB13th pin ,positon is 27,26  (01)
	GPIOB_ODR |=(0x01<<13);			//set RED LED to OFF State
	GPIOB_ODR |=(0x01<<14); 
}
void GPIOC_CONFIG(void)
{
	GPIOC_PUPDR |=(0X01<<18);		//switch ENTER
GPIOC_PUPDR |=(0X01<<16);		//switch ENTER
...
int main()
{
	...
	GPIOB_CONFIG();
	GPIOC_CONFIG();
	GPIOC_IDR |= 0x01<<8;			//set PC10 to IDR
	GPIOC_IDR |= 0x01<<9;
	while(1)
	{
if(!(GPIOC_IDR & 0x01<<8))		//check PC10 is ON
{
